    ...
    public void setup(SourceResolver resolver, Map objectModel, String src, Parameters par) throws ProcessingException, SAXException, IOException
    {
    	...
    	try
        {
			...
			if(dso.getType() == Constants.ITEM)
			{
				...
				limit = printLine(content, "Por medio del presente el", limit);
				content.setFont(PDType1Font.HELVETICA_BOLD, FONT_SIZE);
				limit = printLine(content, "SEDICI", limit);
				content.setFont(PDType1Font.HELVETICA, FONT_SIZE);
				limit = printLine(content, "certifica el depósito en el repositorio institucional de la UNLP de la obra que se detalla a continuación bajo el identificador", limit);
				content.setFont(PDType1Font.HELVETICA_BOLD, FONT_SIZE);
