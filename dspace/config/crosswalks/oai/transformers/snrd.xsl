...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/doc:metadata">
		<doc:metadata>
			...
			<xsl:variable name="subtype" select="doc:element[@name='sedici']/doc:element[@name='subtype']/doc:element/doc:field[@name='value']/text()"/>
			<xsl:call-template name="driver-type">
				<xsl:with-param name="subtype" select="$subtype"/>				
			</xsl:call-template>
			<xsl:call-template name="snrd-type">
				<xsl:with-param name="theValue" select="$subtype"/>
			</xsl:call-template>
			...
			<xsl:for-each select="doc:element[@name='sedici']/doc:element[@name='identifier']/doc:element[@name='issn']/doc:element/doc:field">
				<xsl:call-template name="printAlternativeIdentifier">
			...
			<xsl:for-each select="doc:element[@name='sedici']/doc:element[@name='identifier']/doc:element[@name='other']/doc:element/doc:field">
				...
				<xsl:choose>
					...
					<xsl:when test="matches(./text(), '10.\d{4,9}/[-._;()/:A-Z0-9]+', 'i')">	
	...
	<xsl:template name="snrd-type">
		...
		<xsl:variable name="finalValue">
			<xsl:choose>
				...
				<!-- No se exporta
				<xsl:when test="$theValue='Preprint'">
					artículo
				</xsl:when> -->
				<xsl:when test="$theValue='Articulo'">
					artículo
				</xsl:when>
				<xsl:when test="$theValue='Resumen'">
				...
				<xsl:when test="$theValue='Reporte tecnico'">
					informe técnico
				</xsl:when>
				...
				<xsl:when test="$theValue='Capitulo de libro'">
					parte de libro
				</xsl:when>
				...
				<xsl:when test="$theValue='Placa espectrografica'">
					conjunto de datos
				</xsl:when>
				<xsl:when test="$theValue='Placa fotografica'">
					conjunto de datos
				</xsl:when>
				<xsl:when test="$theValue='Proyecto de extension'">
				...
				<!-- No se exportan
				<xsl:when test="$theValue='Documento institucional'">
					otros
				</xsl:when>
				<xsl:when test="$theValue='Resolucion'">
					otros
				</xsl:when>
				<xsl:when test="$theValue='Imagen fija'">
					fotografía
				</xsl:when> 
		...
		<doc:element name="snrd">
			<doc:element name='type'>
				<doc:element name='es'>
					<doc:field name="value"><xsl:value-of select="concat($snrd_type_prefix, normalize-space($finalValue))"/></doc:field>
				</doc:element>
			</doc:element>
		</doc:element>
	</xsl:template>
