...
public class XmlWorkflowManager {
	...
	public static XmlWorkflowItem start(Context context, WorkspaceItem wsi)
		...
		if (firstStep.isValidStep(context, wfi)) {
			...
			firstStep = wf.getNextStep(context, wfi, firstStep,
	...
	public static XmlWorkflowItem startEditItemMetadata(Context context,
			String handle) throws SQLException, IOException,
			WorkflowConfigurationException, AuthorizeException,
			WorkflowException {
		...
		// Creación del wrapper XmlWorkflowItem (seguimos los pasos del start()
		// original, adaptando lo necesario)
		...
		// throw new
		// WorkflowException("El item "+handle+" ya se ecuentra dentro del workflow");
		...
		// TODO Ver si hay que setear el flag withdraw para evitar que el item
		// siga publico
		// item.withdraw();
	...
	public static boolean tienePermisoDeWorkflow(Workflow wf, Item item,
			Context context) {
		...
		try {
			...
			if (firstStep != null) {
				RoleMembers miembros = firstStep.getRole().getMembers(context,
						wfi);
	...
	/**
	 * Ya que a la gente de DSpace se les olvido crear un metodo para retornar
	 * un XmlWorkflowItem según el Item, agregamos uno aqui
	 * 
	 * @param item
	 * @return
	 * @throws SQLException
	 */
	private static boolean workflowItemExists(Context context, Item item)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public static XmlWorkflowItem GetWorkflowItem(Context context, Item item)
			throws SQLException {
		// Look for the unique workspaceitem entry where 'item_id' references
		// this item
		TableRow row = DatabaseManager.findByUnique(context,
		...
		if (row != null) {
			try {
				retorno = XmlWorkflowItem.find(context,
	...
	public static PoolTask GetPoolTask(Context context,
			XmlWorkflowItem workflowItem, EPerson person) throws SQLException {
		PoolTask poolTask = null;
		try {
			poolTask = PoolTask.findByWorkflowIdAndEPerson(context,
					workflowItem.getID(), person.getID());
		} catch (AuthorizeException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
	...
	// TODO: this is currently not used in our notifications. Look at the code
	// used by the original WorkflowManager
	/**
	...
	public static XmlWorkflowItem startWithoutNotify(Context c,
			WorkspaceItem wsi) throws SQLException, AuthorizeException,
			IOException, WorkflowException, WorkflowConfigurationException,
			MessagingException {
	...
	public static void alertUsersOnTaskActivation(Context c,
			XmlWorkflowItem wfi, String emailTemplate, List<EPerson> epa,
			String... arguments) throws IOException, SQLException,
			MessagingException {
		if (noEMail.containsKey(wfi.getItem().getID())) {
			// suppress email, and delete key
			noEMail.remove(wfi.getItem().getID());
		} else {
			Email mail = ConfigurationManager.getEmail(I18nUtil
	...
	private static void grantSubmitterReadPolicies(Context context, Item item)
			throws SQLException, AuthorizeException {
		// A list of policies the user has for this item
		List<Integer> userHasPolicies = new ArrayList<Integer>();
		List<ResourcePolicy> itempols = AuthorizeManager.getPolicies(context,
				item);
	...
	private static void activateFirstStep(Context context, Workflow wf,
			Step firstStep, XmlWorkflowItem wfi) throws AuthorizeException,
			IOException, SQLException, WorkflowException,
			WorkflowConfigurationException {
		WorkflowActionConfig firstActionConfig = firstStep
		...
		if (!firstActionConfig.requiresUI()) {
			ActionResult outcome = firstActionConfig.getProcessingAction()
					.execute(context, wfi, firstStep, null);
			processOutcome(context, null, wf, firstStep, firstActionConfig,
					outcome, wfi, true);
		}
	}
	 ...
	 */
	public static WorkflowActionConfig doState(Context c, EPerson user,
			HttpServletRequest request, int workflowItemId, Workflow workflow,
			WorkflowActionConfig currentActionConfig) throws SQLException,
			AuthorizeException, IOException, MessagingException,
		...
		try {
			...
			if (currentActionConfig.getProcessingAction().isAuthorized(c,
					request, wi)) {
				ActionResult outcome = currentActionConfig
						.getProcessingAction().execute(c, wi, currentStep,
								request);
				return processOutcome(c, user, workflow, currentStep,
						currentActionConfig, outcome, wi, false);
			} else {
				throw new AuthorizeException(
						"You are not allowed to to perform this task.");
			}
		} catch (WorkflowConfigurationException e) {
			log.error(LogManager.getHeader(c, "error while executing state",
					"workflow:  " + workflow.getID() + " action: "
							+ currentActionConfig.getId() + " workflowItemId: "
							+ workflowItemId), e);
	...
	public static WorkflowActionConfig processOutcome(Context c, EPerson user,
			Workflow workflow, Step currentStep,
			WorkflowActionConfig currentActionConfig,
			ActionResult currentOutcome, XmlWorkflowItem wfi,
			boolean enteredNewStep) throws IOException,
			WorkflowConfigurationException, AuthorizeException, SQLException,
			WorkflowException {
		if (currentOutcome.getType() == ActionResult.TYPE.TYPE_PAGE
			...
			// We either pressed the cancel button or got an order to return to
			// the submission page, so don't return an action
			// By not returning an action we ensure ourselfs that we go back to
			// the submission page
			c.restoreAuthSystemState();
			return null;
			...
			if (currentOutcome.getResult() == ActionResult.OUTCOME_COMPLETE) {
				nextActionConfig = currentStep
						.getNextAction(currentActionConfig);
			}
			...
			if (nextActionConfig != null) {
				...
				if (nextActionConfig.requiresUI() && !enteredNewStep) {
					...
					ClaimedTask task = ClaimedTask.findByWorkflowIdAndEPerson(
							c, wfi.getID(), user.getID());
					...
					// We have entered a new step and have encountered a UI,
					// return null since the current user doesn't have anything
					// to do with this
					c.restoreAuthSystemState();
					return null;
				} else {
					ActionResult newOutcome = nextActionConfig
							.getProcessingAction().execute(c, wfi, currentStep,
									null);
					return processOutcome(c, user, workflow, currentStep,
							nextActionConfig, newOutcome, wfi, enteredNewStep);
				}
			} else if (enteredNewStep) {
				// If the user finished his/her step, we keep processing until
				// there is a UI step action or no step at all
				Step nextStep = workflow.getNextStep(c, wfi, currentStep,
						currentOutcome.getResult());
				c.turnOffAuthorisationSystem();
				return processNextStep(c, user, workflow, currentOutcome, wfi,
						nextStep);
			} else {
				//
				ClaimedTask task = ClaimedTask.findByWorkflowIdAndEPerson(c,
						wfi.getID(), user.getID());

				// Check if we have a task for this action (might not be the
				// case with automatic steps)
				// First add it to our list of finished users, since no more
				// actions remain
				...
				if ((currentStep.isFinished(c, wfi) && currentOutcome
					...
					// Delete all the table rows containing the users who
					// performed this task
					...
					return processNextStep(c, user, workflow, currentOutcome,
							wfi, nextStep);
				} else {
					// We are done with our actions so go to the submissions
					// page but remove action ClaimedAction first
					deleteClaimedTask(c, wfi, task);
					c.restoreAuthSystemState();
					return null;
				}
		...
		log.error(LogManager.getHeader(c, "Invalid step outcome",
				"Workflow item id: " + wfi.getID()));
		throw new WorkflowException("Invalid step outcome");
	}

	private static WorkflowActionConfig processNextStep(Context c,
			EPerson user, Workflow workflow, ActionResult currentOutcome,
			XmlWorkflowItem wfi, Step nextStep) throws SQLException,
		...
		if (nextStep != null) {
			...
			if (nextActionConfig.requiresUI()) {
				// Since a new step has been started, stop executing actions
				// once one with a user interface is present.
				c.restoreAuthSystemState();
				return null;
			} else {
				ActionResult newOutcome = nextActionConfig
						.getProcessingAction().execute(c, wfi, nextStep, null);
				c.restoreAuthSystemState();
				return processOutcome(c, user, workflow, nextStep,
						nextActionConfig, newOutcome, wfi, true);
			}
		} else {
			if (currentOutcome.getResult() != ActionResult.OUTCOME_COMPLETE) {
				c.restoreAuthSystemState();
				throw new WorkflowException(
						"No alternate step was found for outcome: "
								+ currentOutcome.getResult());
			}
			archive(c, wfi);
			c.restoreAuthSystemState();
	 ...
	 */
	// SeDiCI: Se modifica este metodo para evitar que haga el Install de un
	// Item
	...
	public static Item archive(Context c, XmlWorkflowItem wfi)
		...
		WorkflowItemRole[] workflowItemRoles = WorkflowItemRole.findAllForItem(
				c, wfi.getID());
		...
		// BEGIN SeDiCI: aca preguntamos si es un Item nuevo en el repositorio o
		// uno existente
		if (item.isArchived()) {
			/*
			 * TODO probar y habilitar parche para #1785 // En lugar de hacer un
			 * Install, no hacemos mas que eliminar el XmlWorkflowItem
			 * (directamente o con un restore, segun el embargo) DCDate
			 * newLiftDate; try{ newLiftDate=EmbargoManager.getEmbargoDate(c,
			 * item); }catch (IllegalArgumentException e) { //liftDate is in the
			 * past or uninterpretable newLiftDate=null; } DCDate oldLiftDate =
			 * getLiftDate(item);
			 * 
			 * if (oldLiftDate != null && newLiftDate == null){ //se anula el
			 * embargo EmbargoManager.liftEmbargo(c, item); }else if
			 * (newLiftDate != null && !newLiftDate.equals(oldLiftDate)){
			 * EmbargoManager.setEmbargo(c, item, newLiftDate);
			 * }else{//newLiftDate.equals(oldLiftDate) //no-op }
			 */
			wfi.deleteWrapper();
		...
		item.clearMetadata(WorkflowRequirementsManager.WORKFLOW_SCHEMA,
	...
	private static void notifyOfArchive(Context c, Item i, Collection coll)
		...
		try {
			...
			// Get title
			DCValue[] titles = i.getMetadata(MetadataSchema.DC_SCHEMA, "title",
					null, Item.ANY);
			String title = "";
			try {
				title = I18nUtil
	 ...
	 *             ...
	 */
	public static void deleteAllTasks(Context c, XmlWorkflowItem wi)
			throws SQLException, AuthorizeException {
		deleteAllPooledTasks(c, wi);

		List<ClaimedTask> allClaimedTasks = ClaimedTask.findByWorkflowId(c,
				wi.getID());
	...
	public static void deleteAllPooledTasks(Context c, XmlWorkflowItem wi)
			throws SQLException, AuthorizeException {
	 ...
	 */
	public static void deletePooledTask(Context c, XmlWorkflowItem wi,
			PoolTask task) throws SQLException, AuthorizeException {
		if (task != null) {
			task.delete();
			if (task.getEpersonID() > 0) {
				removeUserItemPolicies(c, wi.getItem(),
						EPerson.find(c, task.getEpersonID()));
			} else {
				removeGroupItemPolicies(c, wi.getItem(),
						Group.find(c, task.getGroupID()));
			}
		}
	}

	public static void deleteClaimedTask(Context c, XmlWorkflowItem wi,
			ClaimedTask task) throws SQLException, AuthorizeException {
		if (task != null) {
			task.delete();
			removeUserItemPolicies(c, wi.getItem(),
					EPerson.find(c, task.getOwnerID()));
		}
	}
	 ...
	 */
	public static void createPoolTasks(Context context, XmlWorkflowItem wi,
			RoleMembers assignees, Step step, WorkflowActionConfig action)
			throws SQLException, AuthorizeException {
	 ...
	 */
	public static void createOwnedTask(Context c, XmlWorkflowItem wi,
			Step step, WorkflowActionConfig action, EPerson e)
			throws SQLException, AuthorizeException {
	...
	private static void grantUserAllItemPolicies(Context context, Item item,
			EPerson epa) throws AuthorizeException, SQLException {
		if (epa != null) {
			// A list of policies the user has for this item
			List<Integer> userHasPolicies = new ArrayList<Integer>();
			List<ResourcePolicy> itempols = AuthorizeManager.getPolicies(
					context, item);
			for (ResourcePolicy resourcePolicy : itempols) {
	...
	private static void grantGroupAllItemPolicies(Context context, Item item,
			Group group) throws AuthorizeException, SQLException {
		if (group != null) {
			// A list of policies the user has for this item
	...
	private static void addPolicyToItem(Context context, Item item, int type,
			EPerson epa) throws AuthorizeException, SQLException {
		if (epa != null) {
	...
	private static void addGroupPolicyToItem(Context context, Item item,
			int type, Group group) throws AuthorizeException, SQLException {
		if (group != null) {
	...
	private static void removeUserItemPolicies(Context context, Item item,
		...
		if (e != null) {
			...
			for (Bundle bundle : bundles) {
				...
				for (Bitstream bitstream : bitstreams) {
					AuthorizeManager.removeEPersonPolicies(context, bitstream,
							e);
				}
			}
	...
	private static void removeGroupItemPolicies(Context context, Item item,
			Group e) throws SQLException, AuthorizeException {
		if (e != null) {
			// Also remove any lingering authorizations from this user
	...
	public static WorkspaceItem sendWorkflowItemBackSubmission(Context c,
		...
		String provDescription = provenance + " Rejected by " + usersName
				+ ", reason: " + rejection_message + " on " + now + " (GMT) ";
	...
	private static WorkspaceItem returnToWorkspace(Context c,
			XmlWorkflowItem wfi) throws SQLException, IOException,
			AuthorizeException {
		...
		DatabaseManager.updateQuery(
				c,
				"DELETE FROM cwf_workflowitem WHERE workflowitem_id="
						+ wfi.getID());
	...
	private static void recordStart(Item myitem, Action action)
		...
		myitem.addMetadata(MetadataSchema.DC_SCHEMA, "description",
				"provenance", "en", provmessage);
		myitem.update();
	}

	private static void notifyOfReject(Context c, XmlWorkflowItem wi,
		...
		try {
			...
			Email email = ConfigurationManager.getEmail(I18nUtil
					.getEmailFilename(supportedLocale, "submit_reject"));
	...
	private static DCDate getLiftDate(Item item) {
		...
		DCValue[] liftDate = item.getMetadata(lift_schema, lift_element,
				lift_qualifier, Item.ANY);
	...
	public static void cleanWorkflowEdited(Context c, Item item)
			throws SQLException, AuthorizeException {
	...
	public static void setWorkflowEdited(Context c, Item item)
			throws SQLException, AuthorizeException {
		item.clearMetadata(wf_edited_schema, wf_edited_element, null, "es");
		item.addMetadata(wf_edited_schema, wf_edited_element, null, "es",
				"true");
		item.update();
		c.commit();
	}
	...
	public static boolean isWorkflowEdited(Item item) {
		DCValue[] values = item.getMetadata(wf_edited_schema,
				wf_edited_element, null, "es");
		if (values.length > 0)
