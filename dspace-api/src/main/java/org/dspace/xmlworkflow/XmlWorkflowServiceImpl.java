...
public class XmlWorkflowServiceImpl implements XmlWorkflowService {
    ...
    public XmlWorkflowItem  start(Context context, WorkspaceItem wsi) throws SQLException, AuthorizeException, IOException, WorkflowException {
        try {
            ...
            XmlWorkflowItem wfi = xmlWorkflowItemService.findByItem(context, myitem);
        	if( wfi != null ){
        		xmlWorkflowItemService.create(context, myitem, collection);
	    	...
	    	if(!shouldSendAlert(context, wfi))
     ...
     * @throws SQLException
     */
    public boolean shouldSendAlert(Context context, XmlWorkflowItem wfi) throws SQLException {
    	// disables notifications for collection ADMINs
    	if (authorizeService.isAdmin(context) || (wfi.getCollection() == null))
    		return false;
    	else 
    		return !authorizeService.isAdmin(context, wfi.getCollection());
    }
...
//                nextActionConfig.getProcessingAction().generateTasks();
    ...
    private DCDate getLiftDate(Item item ){
        ...
        String liftDate = itemService.getMetadataFirstValue(item, lift_schema, lift_element, lift_qualifier, Item.ANY);
        if (liftDate == null)
        	return null;
        else
        	return new DCDate(liftDate);
    }
    ...
    @Override
    public void cleanWorkflowEdited(Context c, Item item) throws SQLException, AuthorizeException {
    	itemService.clearMetadata(c, item, wf_edited_schema, wf_edited_element, null, "es");
    	itemService.update(c, item);
    }
    	
    @Override
    public void setWorkflowEdited(Context c, Item item) throws SQLException, AuthorizeException {
    	itemService.clearMetadata(c, item, wf_edited_schema, wf_edited_element, null, "es");
    	itemService.addMetadata(c, item, wf_edited_schema, wf_edited_element, null, "es", "true");
    	itemService.update(c, item);
    }

    @Override
    public boolean isWorkflowEdited(Item item) {
    	String value = itemService.getMetadataFirstValue(item, wf_edited_schema, wf_edited_element, null, "es");
    	if(value != null)
